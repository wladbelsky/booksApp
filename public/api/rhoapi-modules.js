...
/*        propsSupport.ffHackKeywords = (function supported_firefoxHack_keywords() {
    ...
    addBridge(rhoPlatform.id.WP8, function() {
     ...
     addBridge(rhoPlatform.id.UWP, function() {
        return {
            apiCall: function (cmdText, async, resultHandler) {
                //try {
                    var apiResult = window.rhoRuntimeInvoker.invoke(cmdText);
                    resultHandler(JSON.parse(apiResult));
                //} catch (e) {}
            }
        }
...
if (typeof(qt) == "undefined"){
    ...
    addBridge(rhoPlatform.id.WIN32, function() {
        return {
            /*apiCall: function (cmdText, async, resultHandler) {
                if (window[RHO_API_TAG] && 'function' == typeof window[RHO_API_TAG]['apiCall']) {
                    var ret = undefined;
                    window[RHO_API_TAG].apiCall(cmdText, async, function(returnValue) {console.log(ret = returnValue);});
                }   
            }*/
            apiCall: ajaxBridgeApiCall
        }
    ...
    addBridge(rhoPlatform.id.SAILFISH, function() {
        return {
            apiCall: ajaxBridgeApiCall
        }
    });
    ...
    var bridgeMapping = [
        ...
        [/Windows\s+Phone\s+10/            , bridges[rhoPlatform.id.UWP]         ],
        [/Android/                         , bridges[rhoPlatform.id.ANDROID]     ],
        [/iPhone|iPod|iPad/                , bridges[rhoPlatform.id.IPHONE]      ],
        [/.*WindowsPhone.*/                , bridges[rhoPlatform.id.UWP]         ],
        [/.*MicrosoftMDG.*/                , bridges[rhoPlatform.id.UWP]         ],
        [/.*WebView.*/                     , bridges[rhoPlatform.id.UWP]         ],
        [/Windows\s+(?:Mobile|CE)|WM [0-9]/, bridges[rhoPlatform.id.WM]          ],
        [/MSIE/                            , bridges[rhoPlatform.id.WM]          ],
        [/Windows/                         , bridges[rhoPlatform.id.WIN32]       ],
        [/(Unknown;\s+Linux)/              , bridges[rhoPlatform.id.SAILFISH]    ]
    ];
...
(function ($, rho, rhoUtil) {
    ...
    rhoUtil.createMethodsProxy(Notification, [
          ...
          // function(/* const rho::Hashtable<rho::String, rho::String>& */ propertyMap, /* optional function */ oResult)
        , { methodName: 'setScheduler', nativeName: 'setScheduler', valueCallbackIndex: 1 }
    
          // function(/* optional function */ oResult)
        , { methodName: 'removeScheduler', nativeName: 'removeScheduler', valueCallbackIndex: 0 }
...
(function ($, rho, rhoUtil) {
    ...
    // === System constants ===
            ...
            System.PLATFORM_SAILFISH = 'SAILFISH'; 
    
            System.PLATFORM_UWP = 'UWP'; 
    ...
    rhoUtil.createPropsProxy(System, [
      ...
      , { propName: 'NodejsServerPort', propAccess: 'r' }
      , { propName: 'freeServerPort', propAccess: 'r' }
      , { propName: 'externalStorageDirectoryPath', propAccess: 'r' }
...
(function ($, rho, rhoUtil) {
    ...
    rhoUtil.createPropsProxy(WebView, [
      ...
      , { propName: 'enableMediaPlaybackWithoutGesture', propAccess: 'r' }
      ...
      , { propName: 'enableDragAndDrop', propAccess: 'rw' }
    ], apiReq);
...
(function ($, rho, rhoUtil) {
    ...
    // === Camera constants ===
            ...
            Camera.STATUS_CANCELLED = 'cancel'; 
    
            Camera.STATUS_ERROR = 'error'; 
